...
_amazon_profile_instance = None

def get_amazon_profile():
    global _amazon_profile_instance
    if _amazon_profile_instance is None:
        _amazon_profile_instance = QWebEngineProfile("amazon")
        _amazon_profile_instance.setHttpUserAgent('Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Safari/537.36')
        
        settings = _amazon_profile_instance.settings()
...
class LoginBrowserDialog(QDialog):
    def __init__(self, start_url, target_cookies=None, success_url=None, api_key_mode=False, parent=None):
        ...
        # However, we must NOT parent the shared persistent Amazon profile, otherwise it gets destroyed when the dialog closes,
        # causing subsequent accesses to crash.
        if api_key_mode != "amazon":
